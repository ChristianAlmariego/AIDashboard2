...
// ADO rejects fields + $expand in the same request — use a separate call for relations
...
export async function fetchUserStories(pat) {
  ...
  // Fetch fields and relations in parallel (must be separate requests)
  ...
  if (taskIdSet.size > 0) {
    ...
    rawTasks.forEach((raw) => { taskMap[raw.id] = normalizeItem(raw); });
  }

  return stories.map((story) => ({
    ...story,
    tasks: (storyTaskIds[story.id] ?? []).map((id) => taskMap[id]).filter(Boolean),
  }));
}
